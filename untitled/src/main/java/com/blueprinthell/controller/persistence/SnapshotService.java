...
import com.blueprinthell.controller.systems.SnapshottableBehavior;
...
public final class SnapshotService {
    ...
    public NetworkSnapshot buildSnapshot() {
        ...
        for (SystemBoxModel b : boxes) {
            ...
            if (b.getBehaviors() != null) {
                for (SystemBehavior behavior : b.getBehaviors()) {
                    if (behavior instanceof SnapshottableBehavior sb) {
                        bs.behaviorStates.put(behavior.getClass().getName(), sb.captureState());
                    }
                }
            }
    ...
    public void restore(NetworkSnapshot snap) {
        ...
        for (BoxState bs : snap.world.boxes) {
            ...
            if (bs.behaviorStates != null && box.getBehaviors() != null) {
                for (SystemBehavior behavior : box.getBehaviors()) {
                    if (behavior instanceof SnapshottableBehavior sb) {
                        Map<String, Object> savedState = bs.behaviorStates.get(behavior.getClass().getName());
                        if (savedState != null) {
                            sb.restoreState(savedState);
                        }
                    }
                }
            }
    ...
    public static PacketState toPacketState(PacketModel p) {
        final PacketModel originalPacket = PacketOps.unwrapTrojan(p);
        final boolean isTrojan = p instanceof TrojanPacket;

        PacketState ps = createBasePacketState(originalPacket);

        // اگر پکت اولیه تروجان بود، اطلاعات مربوط به تروجان را اضافه می‌کنیم
        if (isTrojan) {
            ps.trojanOriginalFamily = ps.family;
            ps.family = "TROJAN";
            ps.trojanOriginalType = originalPacket.getType().name();
        }

        return ps;
    }

    private static PacketState createBasePacketState(PacketModel p) {
        ...
        } else if (p instanceof LargePacket lp) {
            ps.family = "LARGE";
        ...
        } else if (p instanceof ProtectedPacket pp) {
            ps.family = "PROTECTED";
            ps.protectedShield = pp.getShield();
    ...
    public static PacketModel fromPacketState(PacketState ps) {
        if ("TROJAN".equals(ps.family)) {
            PacketState originalState = ps;
            originalState.family = ps.trojanOriginalFamily;

            PacketModel originalPacket = fromPacketState(originalState);

            return PacketOps.toTrojan(originalPacket);
        }
        ...
        if ("BIT".equals(ps.family)) {
            ...
            base = new BitPacket(t, Config.DEFAULT_PACKET_SPEED, gid, psize, idx, col);
        } else if ("MERGED".equals(ps.family)) {
            int gid   = (ps.groupId != null) ? ps.groupId : 0;
            int exp   = (ps.expectedBits != null) ? ps.expectedBits : 0;
            int col   = (ps.colorId != null) ? ps.colorId : 0;
            int psize = (ps.parentSizeUnits != null) ? ps.parentSizeUnits : 0;
            base = new MergedPacket(t, Config.DEFAULT_PACKET_SPEED, psize, gid, exp, col);
        } else if ("LARGE".equals(ps.family)) {
            int psize = (ps.parentSizeUnits != null) ? ps.parentSizeUnits : 0;
            LargePacket lp = new LargePacket(t, Config.DEFAULT_PACKET_SPEED, psize);
            ...
            if (ps.customRgb != null) {
                lp.setCustomColor(new Color(ps.customRgb, true));
            }
            ...
            base = new ProtectedPacket(t, Config.DEFAULT_PACKET_SPEED, shield);
        } else if ("CONFIDENTIAL".equals(ps.family)) {
            boolean vpn = Boolean.TRUE.equals(ps.confidentialVpn);
            // برای حفظ اندازه صحیح، از toConfidential... استفاده می‌کنیم
            PacketModel temp = new PacketModel(t, Config.DEFAULT_PACKET_SPEED);
            base = vpn ? PacketOps.toConfidentialVpn(temp) : PacketOps.toConfidential(temp);
        } else { // MESSENGER
            base = new PacketModel(t, Config.DEFAULT_PACKET_SPEED);
        }
